...
[grid.design.title]
Lay out the black squares: the designer tries hundreds of symmetric patterns, filling each one to see how it holds up, and keeps the one that fills the most different ways, never one with a section hanging off the rest by just one or two squares. The blocks and inked words already on the grid stay where they are, and theme entries are built into the pattern.
